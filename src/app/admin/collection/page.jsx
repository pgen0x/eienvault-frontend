    ...
    await axios
      .request({
        ...
        url: `${process.env.NEXT_PUBLIC_API_URL}/api/collection/${
          search == '' ? 'get' : 'search'
        }?query=${search}&page=${page}&limit=${perPage}`,
        headers: {
          ...
          {data.map((item, index) => {
            return (
              <tr className="text-center" key={index}>
                ...
                <Td>
                  {parseFloat(formatEther(Number(item.floorPrice))).toFixed(2)}
                </Td>
                <Td>
                  {parseFloat(formatEther(Number(item.volume))).toFixed(2)}
                </Td>
                <Td>{item.totalOwners}</Td>
