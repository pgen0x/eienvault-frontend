  ...
  return (
    <form action="/search" onSubmit={(event) => handleSearch(event)} className={`${toggleSearch ? 'absolute left-0 bg-white dark:bg-zinc-800' : ''} w-full`}>
      <div className="w-full inline-flex h-10 items-center justify-start gap-2 rounded-xl border-0 px-4 bg-transparent md:bg-white md:dark:bg-zinc-800">
        <span className="text-xl font-black text-neutral-700 dark:text-zinc-200" onClick={() => setToggleSearch(!toggleSearch)}>
          <FontAwesomeIcon icon={toggleSearch ? faChevronLeft : faSearch} />
        </span>
        <input
          ref={inputRef}
          name="search"
          className={`${toggleSearch ? '' : 'hidden'} h-8 w-full rounded-lg border-0 bg-transparent p-2.5 text-sm text-gray-900 focus:border-0 focus:ring-0 dark:border-neutral-800 dark:bg-zinc-800 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500  dark:focus:ring-blue-500 sm:hidden md:block lg:block xl:block 2xl:block`}
          type="text"
          autoComplete='off'
          placeholder="Search ..."
          aria-label="Search"
        ...
        <div className="hidden sm:hidden md:block lg:block xl:block 2xl:block">
          <div className="inline-flex flex-col items-center justify-center gap-2 rounded-md dark:bg-neutral-700 bg-zinc-200 px-2">
            <div className="text-xs font-light leading-normal text-neutral-700 dark:text-zinc-200 transition delay-200 duration-500 ease-in-out w-12 text-center">
              {shortcutText}
            </div>
          </div>
        </div>
