...
const Onsale = ({
  ...
  const getNfts = async () => {
    ...
    await axios
      .request({
        ...
        url: `${process.env.NEXT_PUBLIC_API_URL}/api/market/itemsbyuseraddress/${userAccount}?query=${search}&page=${onsalePage}`,
      })
      .then((response) => {
        if (response.data.nfts.length > 0) {
          if (onsalePage >= response.data.totalPages) {
            setLastOnsale(true); // Set nftLast to true if the current page is the last page
          }
  ...
  const handleScroll = () => {
    ...
    // Add a check for LastOnsale here to prevent incrementing onsalepage
    if (windowBottom >= docHeight && LastOnsale === false) {
      setOnsalePage((oldPage) => oldPage + 1);
    }
  };
          ...
          <div className="col-span-12 flex flex-col gap-2 md:flex-row">
            ...
            >
              <div className="inline-flex h-10 w-full items-center justify-start gap-2 rounded-full border-0 border-gray-200 bg-white px-4 dark:bg-neutral-900">
                ...
                />
                <div className="inline-flex flex-col items-center justify-center gap-2 rounded-md bg-neutral-200 px-2">
