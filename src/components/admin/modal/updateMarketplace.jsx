...
export default function ModalUpdateMarketplace({
  isOpenModal,
  onClose,
  onModalClose,
}) {
  ...
  const [updateMarketplaceHash, setUpdateMarketplaceHash] = useState(false);
    ...
    try {
      ...
      if (isAdmin) {
        const hash = await walletClient.writeContract({
          ...
          functionName: 'updateMarketAddress',
          args: [data.marketplacecontract],
          account: address,
        });
        setUpdateMarketplaceHash(hash);
      } else {
  ...
  const { data: dataUpdateMarketplace, isError: isErrorUpdateMarketplace } =
    useWaitForTransaction({
      hash: updateMarketplaceHash,
    });

  useEffect(() => {
    const fetchData = async () => {
      if (updateMarketplaceHash) {
        if (isErrorUpdateMarketplace) {
          setIsSubmit(false);
          toast.error('Error Update Marketplace Contract Address');
          throw new Error('Error Update Marketplace Contract Address');
        }

        if (dataUpdateMarketplace) {
    ...
    fetchData();
  }, [dataUpdateMarketplace, updateMarketplaceHash, isErrorUpdateMarketplace]);
                          ...
                          <form>
                            <div className="w-full">
                              <div className="mt-2 w-full">
                                <p>
                                  Please input the new marketplace contract
                                  address
                                </p>
                              </div>
