...
export default function ModalUnpause({
  isOpenModal,
  onClose,
  onModalClose,
  type,
  ...
  const [unpauseHash, setUnpauseHash] = useState(false);
  ...
  const { data: dataUnpause, isError: isErrorUnpause } = useWaitForTransaction({
    hash: unpauseHash,
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (unpauseHash) {
        if (isErrorUnpause) {
          setIsSubmit(false);
          toast.error('Error Unpause Contract');
          throw new Error('Error Unpause Contract');
        }

        if (dataUnpause) {
          toast.success('Unpause Marketplace Contract Success');
        }
      }
    };

    fetchData();
  }, [dataUnpause, unpauseHash, isErrorUnpause]);
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-zinc-800">
                  ...
                  >
                    Are you sure to unpause this contract?
                  </Dialog.Title>
                      ...
                      <div className="text-gray-900 dark:text-white">
                        <section className="flex flex-col gap-2 overflow-y-auto">
                          <div className="mt-2 w-full">
                            <p>
                              If you aggree to unpause this contract check your
