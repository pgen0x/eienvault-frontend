...
const Sold = ({ userAccount, handleOpenModalShare, handleOpenModalReport }) => {
  ...
  const getNfts = async () => {
    ...
    await axios
      .request({
        ...
        url: `${process.env.NEXT_PUBLIC_API_URL}/api/market/itemsoldbyuseraddress/${userAccount}?page=${soldPage}`,
      })
      .then((response) => {
        if (response.data.nfts.length > 0) {
          if (soldPage >= response.data.totalPages) {
