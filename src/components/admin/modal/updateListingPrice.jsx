...
export default function ModalUpdateListingPrice({
  ...
  const [updateListingPriceHash, setUpdateListingPriceHash] = useState();
    ...
    try {
      ...
      if (isAdmin) {
        const hash = await walletClient.writeContract({
          ...
          args: [parseEther(data.listingprice)],
          account: address,
        });
        setUpdateListingPriceHash(hash);
      } else {
  ...
  const { data: dataUpdateCommission, isError: isErrorUpdateCommission } =
    useWaitForTransaction({
      hash: updateListingPriceHash,
    });

  useEffect(() => {
    const fetchData = async () => {
      if (updateListingPriceHash) {
        if (isErrorUpdateCommission) {
          setIsSubmit(false);
          toast.error('Error Update Listing Price');
          throw new Error('Error Update Listing Price');
        }

        if (dataUpdateCommission) {
          toast.success('Update Listing Price Success');
        }
      }
    };

    fetchData();
  }, [dataUpdateCommission, updateListingPriceHash, isErrorUpdateCommission]);
                          ...
                          <form>
                            <div className="w-full">
                              <div className="mt-2 w-full">
                                <p>Please input the listing price</p>
                              </div>
                              <div className="mt-4 w-full">
                                ...
                                <div className="mt-2 flex w-full items-center rounded-full border border-gray-200 bg-white dark:text-gray-900">
                                  ...
                                  <span className="flex gap-2">
                                    <HelaIcon className="mr-2 h-6 w-6" />
                                  </span>
                                </div>
                            ...
                            >
                              {isSubmit ? 'Loading...' : 'Update Listing Price'}
                            </button>
                          </form>
                        </section>
                      </div>
